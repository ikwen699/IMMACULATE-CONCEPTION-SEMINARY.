    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" role="dialog" aria-modal="true" aria-labelledby="fill-title">
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl animate-scale-in">
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Score (max {max})</label>
            <input
              type="number"
              min="0"
              max={max}
              ...
              autoFocus
              className="w-full px-3.5 py-3 bg-gray-50/80 border border-gray-200 rounded-xl text-lg text-center tabular-nums text-gray-900 focus:outline-none focus:ring-2 focus:ring-violet-500/25 focus:border-violet-400 transition-colors placeholder:text-gray-300"
              placeholder={`0 — ${max}`}
            />
            {isExam && (
              <p className="mt-2 text-[11px] text-gray-400 flex items-center gap-1">
                <span className="mdi mdi-information-outline text-sm text-violet-400" />
          ...
          <div>
            <p className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Quick fill</p>
            <div className="flex flex-wrap gap-1.5">
              ...
              <button
                onClick={() => setValue('')}
                className="px-3 py-1.5 rounded-lg text-sm font-medium text-gray-400 bg-gray-50 hover:bg-gray-100 transition-colors"
              >
                Clear
              </button>
            </div>
